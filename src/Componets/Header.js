      ...
      <div className="container mx-auto px-3 sm:px-4 py-2 sm:py-3">
        <nav className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2 sm:space-x-3">
            ...
            {user ? (
              // Logged in - Compact on mobile, full on desktop
              <div className="hidden md:flex items-center space-x-3 bg-white/5 backdrop-blur-sm px-3 py-1 rounded-full border border-white/10">
                ...
                <button
                  onClick={logout}
                  className="bg-gradient-to-r from-yellow-400 to-orange-500 text-gray-900 px-3 py-1 rounded-full font-semibold text-xs shadow-md hover:scale-105 hover:from-yellow-500 hover:to-orange-600"
                >
                  Logout
                </button>
              </div>
            ) : (
              // Not logged in - Show Login button on all sizes
              <button
                onClick={handleLogin}
        ...
        {menuOpen && (
          <div className="md:hidden mt-2 sm:mt-3 bg-indigo-950/95 backdrop-blur-xl rounded-xl sm:rounded-2xl border border-white/10 p-3 sm:p-4 space-y-3 sm:space-y-4 animate-fadeIn">
            ...
            {/* Mobile Auth - Only show if needed, but since button is now outside, perhaps remove or keep for consistency */}
            {user && (
              <div className="flex justify-center pt-2">
                <button
                  ...
                  className="bg-gradient-to-r from-yellow-400 to-orange-500 text-gray-900 px-4 py-1.5 rounded-full font-semibold text-sm shadow-md hover:scale-105 transition-all duration-300 w-full sm:w-auto"
                >
